...
from tests.utils import (get_kibon_donjak_by_id, invalid_uuid,
                         kibon_donjak_valid)
...
class TestKibonDonjak:
    ...
    def test_post_kibon_donjak_existing_name(
        self, client: TestClient, kibon_donjak
    ):
        client.post(url, json=kibon_donjak)
        response = client.post(url, json=kibon_donjak)
        assert response.status_code == HTTPStatus.CONFLICT
        assert (
            response.json()['message'] == f"{kibon_donjak['name']} já existe."
        )

    def test_post_kibon_donjak_with_wrong_name(
        self, client: TestClient, kibon_donjak
    ):
    ...
    def test_post_kibon_donjak_with_wrong_meaning(
        self, client: TestClient, kibon_donjak
    ):
    ...
    def test_update_band(self, client: TestClient, kibon_donjak):
        ...
        response = client.put(
            f"{url}{kibon_donjak_caught['id']}", json=kibon_donjak
        )
        assert response.status_code == HTTPStatus.OK
        new_kibon_donjak_caught = client.get(
            f'{url}{kibon_donjak_caught["id"]}'
        )
        assert new_kibon_donjak_caught.json()['name'] == kibon_donjak['name']
        assert (
            new_kibon_donjak_caught.json()['updated_at']
            != kibon_donjak_caught['updated_at']
        )
